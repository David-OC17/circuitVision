...
// Variables
GPIO_Type *d0_gpio, *d1_gpio, *d2_gpio, *d3_gpio, *d4_gpio, *d5_gpio, *d6_gpio,
    *d7_gpio;
...
void LCD8_Clear(void) { LCD8_Command(LCD8_CLEAR_COMMAND); }

// Set LCD cursor to No blink
void LCD8_NoBlink(void) { LCD8_Command(LCD8_NO_BLINK); }

// Set cursor to blink
void LCD8_Blink(void) { LCD8_Command(LCD8_BLINK_HOME); }

void LCD8_Write(char *data) {
  while (*data !=
         '\0') // Loop through all the string, until null-terminator is found
  {
