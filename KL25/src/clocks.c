...
void ClkFreqConfig(uint32_t clock_freq) {
  SysTick->LOAD =
      clock_freq;        // Remember default systick freq for KL25Z is 21.9 Mhz
  SysTick->CTRL |= 0x05; // Enable timer and set sysclk as source clock
}

// Get current systicks
uint64_t GetSysTicks(void) { return systicks; }

// Set delay ticks in current timing unit
void SetDelayTicks(uint32_t delay_ticks) { time_delay = delay_ticks; }
