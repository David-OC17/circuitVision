...
Stepper CreateStepper(uint32_t stp_pin, PORT_Type *stp_port, uint32_t dir_pin,
                      PORT_Type *dir_port, uint32_t enable_pin,
...
void SetStepperSettings(Stepper *stepper, uint8_t dir, uint32_t step_delay,
                        uint16_t steps_rev) {
...
// Put this function inside a LOOP to move constantly (note that systicks should
// be less than millis)
...
void SetPinHigh(GPIO_Type *port, uint32_t pin) { port->PDOR |= pin; }

void SetPinLow(GPIO_Type *port, uint32_t pin) { port->PDOR &= ~pin; }
...
static uint32_t ToBinaryGPIO(uint32_t num) { return (1 << num); }
